...
void nbd_object::primary_time_advance(double del_t)
{	
	vector<double> F_0_t0=scaling_vector(this->F_0,this->m*G);
	vector<double> F_1_t0=scaling_vector(this->F_1,this->m*G);
	//printf("Force in advance timestep is %3.10f,%3.10f,%3.10f\n",F[0],F[1],F[2]);
	vector<double> v_new=elementwise_sum(scaling_vector(F_0_t0, del_t),scaling_vector(F_1_t0,0.5*del_t*del_t));
	v_new=elementwise_sum(v_new, this->v);
	vector<double> r_new=scaling_vector(F_1_t0,0.166*del_t*del_t*del_t);
	r_new=elementwise_sum(r_new,scaling_vector(F_0_t0, 0.5*del_t*del_t));
...
{
	F_0_t0=scaling_vector(this->F_0_t0,this->m*G);
	F_1_t0=scaling_vector(this->F_1_t0,this->m*G);
	vector<double> F_0=scaling_vector(this->F_0, this->m*G);
	vector<double> F_1=scaling_vector(this->F_1, this->m*G);
	...
	//printf("Force in advance timestep is %3.10f,%3.10f,%3.10f\n",F[0],F[1],F[2]);
	vector<double> v_corr=elementwise_sum(scaling_vector(F_2_t0, 0.166666*del_t*del_t*del_t),scaling_vector(F_3_t0,0.04166666*del_t*del_t*del_t));
	this->v=elementwise_sum(v_corr, this->v);
	vector<double> r_corr=elementwise_sum(scaling_vector(F_2_t0,0.04166666*del_t*del_t*del_t*del_t),scaling_vector(F_3_t0,0.00833333*del_t*del_t*del_t*del_t*del_t));
